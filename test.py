...
for idx_x, line in enumerate(word_search_txt.splitlines()):
    # print(line)
    for idx_y, char in enumerate(line):
        ...
        # )
        word_search_list[idx_x][idx_y] = char
...
found_msg = None
found = []
for idx_x, i in enumerate(word_search_list):
...
def len_is_ok(x):
    return x < len(word_search_list)


def search_match(current_idx_pair, letter_idx):
    # print(
    ...
    match = False
    idx_pair_list.append(current_idx_pair)
    for x, y in get_neighbour_idx(*current_idx_pair):
        # print("->" * letter_idx, x, y, word_search_list[x][y])
        if len_is_ok(x) and len_is_ok(y) and word_search_list[x][y] == text_to_search[letter_idx]:
            # if word_search_list[x][y] == text_to_search[letter_idx]:
            # letter_idx+1

            if letter_idx > 2 and colliniear(*idx_pair_list[0], *idx_pair_list[1], x, y):
                print("->" * letter_idx, x, y, word_search_list[x][y])
                if letter_idx + 1 < len(text_to_search):
                    match = search_match([x, y], letter_idx + 1)
                else:
                    idx_pair_list.append([x, y])
                    return True
...
for idx_pair in idx_pair_list:
    # print(idx_pair)
    x, y = idx_pair
    print(word_search_list[x][y], end="")
print()
